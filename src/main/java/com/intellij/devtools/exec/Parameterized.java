...
public interface Parameterized {
  ...
  default boolean configureParameters(JPanel parametersPanel) {
    ...
    parameterGroups.forEach(parameterGroup -> {
      JPanel row = new JPanel(new GridLayoutManager(1, 2 * parameterGroup.getParameters().size()));
      ...
      parameterGroup.getParameters().forEach(parameter -> {
        ...
        if(componentHolder.isPresent()) {
          ...
          row.add(label, buildGridConstraint(0, j.get(), 1, 1, FILL_HORIZONTAL, SIZEPOLICY_CAN_SHRINK));
          row.add(component, buildGridConstraint(0, j.get() + 1, 1, 1, FILL_HORIZONTAL, SIZEPOLICY_CAN_SHRINK));
          j.getAndAdd(2);
