...
public class ProjectSteps {
  ...
  public void createNewJavaProject() {
    step(
        "Create New Project",
        () -> {
          final WelcomeFrameFixture welcomeFrame =
              remoteRobot.find(WelcomeFrameFixture.class, TEN_SECONDS.getDuration());
          welcomeFrame.createNewProjectLink().click();

          final DialogFixture newProjectDialog =
              welcomeFrame.find(
                  DialogFixture.class,
                  DialogFixture.byTitle("New Project"),
                  TEN_SECONDS.getDuration());
          newProjectDialog
              .find(JListFixture.class, byXpath("//div[@class='JBList']"))
              .clickItem("New Project", true);
              ...
              () ->
                  remoteRobot
                          .findAll(
                              DialogFixture.class,
                              byXpath(
                                  "//div[@class='MyDialog'][.//div[@text='Running startup activities...']]"))
                          .size()
                      == 0);
          //      final IdeaFrame idea = remoteRobot.find(IdeaFrame.class, ofSeconds(10));
          //      idea.dumbAware(() -> {
          //        try {
          //          idea.find(DialogFixture.class, byTitle("Tip of the
          // Day")).button("Close").click();
  ...
  public void openProject() {
    step(
        "Open Project",
        () -> {
          JListFixture projectListFixture =
              remoteRobot.find(
                  JListFixture.class,
                  byXpath("//div[@class='MyList']"),
                  FIVE_SECONDS.getDuration());
